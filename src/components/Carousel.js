...
const Carousel = ({
  ...
  let photos = collection.photos.map((photo, index) => {
    ...
    return (
      ...
      >
        <img
          src={newImage}
          className="d-block w-100 img-fluid rounded-top carousel-photo"
          style={{ width: "100%" }}
          alt="..."
        />
              ...
              onClick={async () => {
                try {
                  if (collection.photos.length <= 1) {
                    addErrorMessage(
                      "A collection must have at least one photo"
                    );
                    return;
                  }
                  await deleteCollectionPhoto(
                    collection._id,
                    photo.imageId.split("/")[1]
                  );
  ...
  let indicators = collection.photos.map((photo, index) => {
    return (
      <li
        key={photo._id}
        data-target={`#carousel-${collection._id}`}
        data-slide-to={index}
