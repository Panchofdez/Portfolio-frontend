...
class CollectionForm extends Component {
  constructor(props) {
    ...
    this.state = {
      title: this.props.location.state
        ? this.props.location.state.collection.title
        : "",
      description: this.props.location.state
        ? this.props.location.state.collection.description
        : "",
      id: 1,
      photo: null,
      showPhoto: null,
  ...
  handleSubmit = async (e) => {
    ...
    try {
      if (this.props.location.state) {
        await this.props.editCollection(
          formData,
          this.props.location.state.collection._id
        );
        ...
        const newArr = this.state.collections.concat({
          title: this.state.title,
          id: this.state.id,
        });
        this.setState({
  ...
  render() {
    ...
    const collectionsAdded = collections.map((collection) => (
      <div key={collection.id} className="alert alert-success">
        Successfully added: {collection.title}
      </div>
    ));
    let collection = null;
    if (this.props.location.state) {
                ...
                {this.props.collection ? (
                  ...
                  <React.Fragment>
                    <label htmlFor="upload-image">
                      Add a photo to your collection
                    </label>
                    <p>
                      <small>You can always add more later</small>
                    </p>
                  </React.Fragment>
                )}
                ...
                {showPhoto ? (
                  <div className="justify-content-center row my-3">
                    <div className="col-12">
                      <img
                        src={showPhoto}
                        className="img-fluid rounded carousel-photo"
                        style={{ width: "100%" }}
                        alt=""
                      />
                    </div>
