...
class SkillsForm extends Component{
	constructor(props){
		...
		this.state={
			skill:"",
			skillsArr:(this.props.location.state && this.props.location.state.portfolio.skills) ? this.props.location.state.portfolio.skills: []
		}
	};
	handleChange=(e)=>{
	...
	handleAdd =(e)=>{
		e.preventDefault();
		this.setState({skillsArr:[...this.state.skillsArr, this.state.skill], skill:""});
	};
	handleRemove=(skillIndex)=>{
		const newArr=this.state.skillsArr.filter((s, index)=>index!== skillIndex);
	...
	render(){
		const skills = this.state.skillsArr.map((s, index)=>{
			return(
				<button 
					key={index} 
					className="btn btn-outline-light rounded-pill my-2 mx-1"
					onClick={()=>this.handleRemove(index)}
				>
					<i className="fas fa-times mx-2"></i>{s}
				</button>
			)
		})
		return(
			<div className="row justify-content-center mt-5">
				<div className="col-md-8 col-10">
					<h2 className="my-3">Add a list of your skills and/or services</h2>					
