...
class ProfileForm extends Component {
  constructor(props) {
    super(props);
    this.state = {
      headerImage: this.props.location.state
        ? this.props.location.state.portfolio.headerImage
        : null,
      showHeaderImage: this.props.location.state
        ? fixImage(this.props.location.state.portfolio.headerImage)
        : null,
      profilePic: this.props.location.state
        ? this.props.location.state.portfolio.profileImage
        : null,
      showProfilePic: this.props.location.state
        ? fixImage(this.props.location.state.portfolio.profileImage)
        : null,
      name: this.props.location.state
        ? this.props.location.state.portfolio.name
  ...
  handleFileChange = (e) => {
    ...
    if (e.target.name === "headerImage" && e.target.files[0]) {
      this.setState({
        showHeaderImage: URL.createObjectURL(e.target.files[0]),
      });
    }
  ...
  handleEdit = async () => {
    try {
      ...
      if (
        ...
        const coverPhotoResponse = await cloudinaryUpload(
          this.state.headerImage
        );
        formData = {
          name: this.state.name,
            ...
            <div className="col-md-8 col-10">
              ...
              <div className="form-group">
                ...
                <div className="input-group ">
                  <div className="custom-file mb-3 elevated p-0">
                    <input
                      type="file"
                      name="profilePic"
                ...
                <div className="input-group mb-3" id="upload-header-image">
                  <div className="custom-file elevated">
                    <input
                      type="file"
                      name="headerImage"
                      className="custom-file-input elevated"
                      id="header-image"
                      onChange={this.handleFileChange}
                    />
                    {headerImage ? (
...
export default connect(null, { createProfile, editProfile, addErrorMessage })(
  ProfileForm
);
