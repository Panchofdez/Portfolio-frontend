				  ...
				  <div class="media-body">
				    ...
				    {this.props.currentUser===comment.author.id && (
						<button 
							...
							onClick={async()=>{
								try{
									await this.props.deleteComment(this.props.portfolio._id, comment._id); 											this.notifyDelete("Deleted comment")
								}catch(err){
									console.log(err);
									return;
								}
							...
							<div>
								<form onSubmit={this.handleSubmit}>
									<textarea 
										...
										rows="3"
									/>
									<button className="btn button mb-3" type="Submit"><i className="fas fa-plus"></i> Post</button>
								</form>
							</div>
